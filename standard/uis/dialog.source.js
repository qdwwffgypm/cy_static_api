(function(doc, win) {
	...
	var dialog = (function() {
		...
		var htmlTeml = {
			box : function() {
				var _box = [];
				_box
						.push('<div class="cy-dialog cy-dialog-{type} cy-dialog-{className}">');
				_box.push('<div class="cy-dialog-top">{top}</div>');
				_box
						.push('<div class="cy-dialog-midddle cy-dialog-{textAlign}">{middle}</div>');
		...
		var _initCss = function() {
			var cssArr = [];
			cssArr.push('.cy-dialog{font-family: "PingFang SC", "Helvetica Neue", Helvetica, STHeitiSC-Light, WOL_SB, Tahoma, Helvetica, sans-serif;z-index:1111;position:fixed;border-radius:13px;background:rgba(255,255,255,.95);overflow:hidden;font-size:17px;line-height:21px;top:50%;left:50%;transform:translate3d(-50%,-50%,0)}');
			cssArr.push('.cy-dialog .cy-dialog-top{font-size:18px;font-weight:500;text-align:center;margin:15px 15px 0 15px;display:flex}');
			cssArr.push('.cy-dialog .cy-dialog-midddle{margin:8px 15px 10px 15px;font-size:14px}');
			cssArr.push('.cy-dialog .cy-dialog-bottom{border-top:1px solid rgba(0,0,0,.2);font-size:17px;display:flex}');
			...
			cssArr.push('.cy-dialog .cy-dialog-btn{color: #D5A940;height:44px;line-height:44px;display:block;width:100%;text-align:center;border-right: 1px solid rgba(0,0,0,.2);}');
			cssArr.push('.cy-dialog .cy-dialog-btn:last-child{border-right:none}');
			...
			cssArr.push('.cy-dialog .cy-dialog-btn-cancel{color: #00000078;}');
		...
		var _initParams = function(args) {
			var args0 = args[0];
			var args1 = args[1];
			typeof args0 == "string" && (args0 = {
				content : args0,
				callback : args[1] || _emptyFun
			});
			typeof args0 == "function" && (args0 = {
				callback : args[0]
			});
			_params = {};
	...
	var toast = (function() {
	    ...
	    var _initCss = function() {
			var cssArr = [];
			cssArr.push('.toast-container{line-height:17px;position:fixed;z-index:9999;bottom:50px;left:50%;-webkit-transition:opacity.3s;transition:opacity.3s;-webkit-transform:translate(-50%,0);transform:translate(-50%,0);opacity:0}');
